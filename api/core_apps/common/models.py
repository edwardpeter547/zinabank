...
from core_apps.user_auth.models import User as CustomUser
...
class ContentView(TimeStampedModel):
    ...
    @classmethod
    def record_view(cls, content_object: Any, user: Optional[CustomUser], viewer_ip: Optional[str]) -> None:
        content_type = ContentType.objects.get_for_model(content_object)
        try:
            view, created = cls.objects.get_or_create(
                content_type=content_type,
                object_id=content_type.id,
                defaults={
                    "user": user,
                    "viewer_ip": viewer_ip,
