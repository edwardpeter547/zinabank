...
class Profile(TimeStampedModel):
    ...
    city = models.CharField(_("City"), max_length=50, default="Unkown")
    ...
    annual_income = models.DecimalField(
        ...
        default=settings.DEFAULT_AMOUNT
    )
    ...
    id_photo = CloudinaryField(_("ID Photo"), null=True, blank=True)
    ...
    def clean(self) -> None:
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_issue_date:
                raise ValidationError("ID expiry date must be before the issue date")
...
class NextOfKin(TimeStampedModel):
    ...
    def __str__(self):
        return f"{self.fullname} - Next of Kin for {self.profile.user.full_name}"
