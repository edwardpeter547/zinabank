...
class User(AbstractUser):
    class SecurityQuestions(models.TextChoices):
        ...
        CHILDHOOD_FRIEND = ("childhood_friend", _("What is the name of you childhood best friend"))
    ...
    def handle_failed_login_attempts(self) -> None:
        ...
        if self.failed_login_attempts >= settings.LOGIN_ATTEMPTS:
            ...
            self.save()
            send_account_locked_email(user=self)
        self.save()
    ...
    def is_locked_out(self) -> bool:
        if self.account_status == self.AccountStatus.LOCKED:
            if (self.last_failed_login 
                and (timezone.now() - self.last_failed_login) > settings.LOCKOUT_DURATION):
                self.unlock_account()
                return False
            return True
        return False
